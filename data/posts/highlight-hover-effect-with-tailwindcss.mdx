...
On the web you might come across a hover effect where the element you are hovering has full opacity and the sibling elements have a lower opacity. It might look something like this.

![](/images/posts/highlight-hover-example.jpg)
...
That's the base for any hover effect like this with Tailwind CSS, now we can take that logic and apply it to a more built out design like this one:

[Highlight Hover Example](https://play.tailwindcss.com/rc93nyfqOu?layout=preview)

---

One thing you'll notice is when hovering in the gaps between the `li` elements that one of them will be on hover, this is to ensure one element is highlighted when the cursor is within the `ul`.
