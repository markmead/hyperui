---
title: Announcements
description: Discover announcement components built with Tailwind CSS v4. These responsive,
  accessible banners and notification bars are perfect for marketing websites,
  SaaS products, and eCommerce stores. Easily highlight updates, promotions, or
  important messages with customizable UI elements optimized for SEO and user
  engagement.
emoji: 📣
category: marketing
slug: announcements
wrapper: h-[200px]
terms:
  - announcement
  - banner
  - notification
components:
  - { title: 'Base', dark: { contributors: ['WinnyChang'] } }
  - { title: 'Base with dismiss', dark: { contributors: ['WinnyChang'] } }
  - { title: 'Fixed', dark: { contributors: ['WinnyChang'] } }
  - { title: 'Fixed with dismiss', dark: { contributors: ['WinnyChang'] } }
  - { title: 'Floating', dark: { contributors: ['WinnyChang'] } }
  - { title: 'Floating with dismiss', dark: { contributors: ['WinnyChang'] } }
---

import CollectionHeader from '../../../components/CollectionHeader.astro'

<CollectionHeader>
  # Announcement Components

  <p>{frontmatter.description}</p>
</CollectionHeader>
