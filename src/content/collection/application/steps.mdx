---
title: Steps
description: Step indicator components for onboarding, progress tracking, and multi-step forms in web applications.
category: application
slug: steps
wrapper: h-[200px]
# pattern: https://js.hyperui.dev/patterns/stepper/
terms:
  - breadcrumb
  - progress
  - navigation
components:
  - { title: 'Title, icon and progress bar', darkContributors: ['WinnyChang'] }
  - { title: 'Title, step/total and progress bar', darkContributors: ['WinnyChang'] }
  - { title: 'Title, check icon and progress bar', darkContributors: ['WinnyChang'] }
  - {
      title: 'Grouped with title and description, with highlighted current step',
      darkContributors: ['WinnyChang'],
    }
  - {
      title: 'Timeline with title, with highlighted current step',
      darkContributors: ['WinnyChang'],
    }
---

import BaseHero from '../../../components/BaseHero.astro'
import BackLink from '../../../components/BackLink.astro'

<BaseHero title={frontmatter.title} description={frontmatter.description}>
  <BackLink
    slot="back"
    href={`/components/${frontmatter.category}`}
    label={`Back to ${frontmatter.category} components`}
  />
</BaseHero>
