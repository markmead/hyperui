---
title: Modals
description: Accessible modal components built with Tailwind CSS v4. Includes simple
  modals, close buttons, and action modals for web applications. Responsive,
  customizable, and optimized for SEO, accessibility, and user experience.
emoji: 🗯️
category: application
slug: modals
terms:
  - modal
  - dialog
  - popup
components:
  - { title: 'Base', dark: true }
  - { title: 'Base with close', dark: true }
  - { title: 'Actions', dark: true }
  - { title: 'Actions with close', dark: true }
  - { title: 'Input', dark: true }
  - { title: 'Input with close', dark: true }
---

import CollectionHeader from '../../../components/CollectionHeader.astro'

<CollectionHeader>
  # Modal Components

  <p>{frontmatter.description}</p>
</CollectionHeader>
