...
I've known I wanted to rewrite HyperUI for a few months now, however, I wanted the rewrite of HyperUI to have meaning and to provide a benefit to myself and contributors.

I believe, that as developers we fall into the trap of believing that we need to constantly rewrite what we've done. As an example of that, I've had three different personal websites in a year which when stripped back, all do the same thing, and if anything the latest version does less than the first version.

However, that no longer applies. I want changes to have meaning and progress the projects (in this case HyperUI) forward.

So, what was the reason behind the HyperUI rewrite...

> HyperUI is a mixture of React and Typescript code that I've written to match what I've seen on other projects, code that has come from other projects (Next JS starter kit) and code that I've written how I'd like it to be written but with limitations so it matches the rest of the project.

Basically, the React and Typescript code of HyperUI didn't feel like it was mine. Here's how I changed that... 🥁
