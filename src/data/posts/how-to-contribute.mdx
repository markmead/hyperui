...
You can see examples of point 3 in the HTML of this [input component](/components/application/pagination#component-3).
...
### Next Console Warnings

When developing locally, you will likely see the console warning `params should be awaited before using its properties`. Please ignore this. We do use `await` but it's for the `getX` functions that take in the `params` and returns resolved data. It does not return an unresolved `Promise`.
